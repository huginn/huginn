module Agents
  class TriggerAgent < Agent
    ...
    description <<-MD
      ...
      The `rules` array contains hashes of `path`, `value`, and `type`.  The `path` value is a dotted path through a hash in [JSONPaths](http://goessner.net/articles/JsonPath/) syntax. For simple messages, this is the name of the field - for example simply body_text for a body_text event.
