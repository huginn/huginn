...
module Agents
  class CoffeeScriptAgent < Agent
    default_schedule "never"

    description <<-MD
      This Agent allows you to write code in CoffeeScript that can create and receive events.  If other Agents aren't meeting your needs, try this one!
    ...
    def default_options
      js_code = <<-JS
        Agent.check = ->
          if @options('make_event')
            @createEvent message: 'I made an event!'
        
        Agent.receive = ->
          events = @incomingEvents()
          for e in events
            @createEvent message: 'I got an event!', event_was: e.payload
      JS
